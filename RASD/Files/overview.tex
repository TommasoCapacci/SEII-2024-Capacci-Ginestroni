...
\begin{enumerate}[label=$\bullet$ \textbf{D\arabic*:}]
    ...
    \item Users are expected to engage in code kata battles with integrity, without resorting to plagiarism or cheating.
    \item Maximum number of students per groups an educator can set will always be bounded (e.g. less than 4).
    \item Students will fork the code kata GitHub repository once ready and invite only his teammates as collaborators.
